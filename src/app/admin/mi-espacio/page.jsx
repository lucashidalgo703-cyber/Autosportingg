...
function AssetActionModal({ isOpen, onClose, onSubmit, saving, error }) {
    const [formData, setFormData] = useState({
        type: 'auto',
        title: '',
        brand: '',
    ...
    const handleSubmit = (event) => {
        ...
        onSubmit({
            ...formData,
            title: formData.title || `${formData.brand} ${formData.model}`.trim() || 'Auto',
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative z-10 flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-crm-border bg-crm-surface shadow-2xl">
                <div className="flex items-start justify-between border-b border-crm-border p-5">
                    <div>
                        <h2 className="m-0 text-lg font-bold text-crm-fg">Nuevo Auto Personal</h2>
                        <p className="m-0 mt-1 text-sm text-crm-fg-muted">Registrá tu vehículo personal. Solo vos podés verlo, no aparece en stock comercial.</p>
                    </div>
                    <button type="button" onClick={onClose} className="rounded-lg px-3 py-2 text-crm-fg-muted hover:bg-crm-surface-raised hover:text-crm-fg">×</button>
                </div>
                <form onSubmit={handleSubmit} className="space-y-4 overflow-y-auto p-5">
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div>
                            <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-crm-fg-muted">Marca</label>
                            <input
                                required
                        ...
                        <div>
                            <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-crm-fg-muted">Modelo</label>
                            <input
                                required
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div>
                            <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-crm-fg-muted">Año</label>
                            <input
                                type="number"
                                min="1900"
                                max="2099"
                        ...
                        <div>
                            <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-crm-fg-muted">Patente</label>
                            <input
                                value={formData.plate}
                                onChange={(event) => setFormData({ ...formData, plate: event.target.value })}
                                className="w-full rounded-xl border border-crm-border bg-crm-bg px-4 py-3 text-sm text-crm-fg outline-none transition-colors focus:border-crm-red"
                                placeholder="Ej: AF123YZ"
                            />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-[1fr_140px]">
                        <div>
                            <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-crm-fg-muted">Valor Estimado</label>
                            <input
                                required
                                type="number"
                                min="0"
                    ...
                    <div>
                        <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-crm-fg-muted">Notas (opcional)</label>
                        <textarea
                            rows="2"
                            value={formData.notes}
                            onChange={(event) => setFormData({ ...formData, notes: event.target.value })}
...
export default function MiEspacioPage() {
    ...
    const { fetchAssets, createAsset, updateAsset, deleteAsset } = useAdminPersonalAssets();
    ...
    const refreshMiSpaceData = async () => {
        const [cars, tasks, installments, sales, transactions, assets] = await Promise.all([
    ...
    const summary = useMemo(() => {
        ...
        const overdueInstallmentAlerts = unpaidInstallments.filter(i => {
            ...
            href: `/admin/finanzas?tab=cuotas`
        }));
        ...
        // --- PATRIMONIO CALCULATIONS ---
        const realAssets = data.assets || [];
        const personalCars = realAssets.filter(a => a.type === 'auto');
        const personalOtherAssets = realAssets.filter(a => a.type === 'otro');
        
        // Sums
        const totalAssetsARS = realAssets.filter(a => a.currency === 'ARS').reduce((acc, a) => acc + (a.estimatedValue || 0), 0);
        const totalAssetsUSD = realAssets.filter(a => a.currency === 'USD').reduce((acc, a) => acc + (a.estimatedValue || 0), 0);

        const parseAmount = (text) => {
            const match = String(text || '').match(/Monto:\s*([A-Z]{3})\s*([0-9.]+)/);
            if (match) return { currency: match[1], amount: Number(match[2]) };
            return { currency: 'ARS', amount: 0 };
        };

        const sumTasks = (tasks, currency) => tasks.reduce((acc, task) => {
            const p = parseAmount(task.description);
            return acc + (p.currency === currency ? p.amount : 0);
        }, 0);

        const toCollectARS = sumTasks(personalTasks.installmentCollect || [], 'ARS');
        const toCollectUSD = sumTasks(personalTasks.installmentCollect || [], 'USD');

        const debtsARS = sumTasks(personalTasks.debt || [], 'ARS');
        const debtsUSD = sumTasks(personalTasks.debt || [], 'USD');

        const payARS = sumTasks(personalTasks.installmentPay || [], 'ARS');
        const payUSD = sumTasks(personalTasks.installmentPay || [], 'USD');
        
        const fixedARS = sumTasks(personalTasks.fixedExpense || [], 'ARS');
        const fixedUSD = sumTasks(personalTasks.fixedExpense || [], 'USD');

        const patrimonioNetoARS = totalAssetsARS + toCollectARS - debtsARS - payARS;
        const patrimonioNetoUSD = totalAssetsUSD + toCollectUSD - debtsUSD - payUSD;
        ...
        return {
            ...
            personalOtherAssets,
            patrimonio: {
                totalAssetsARS, totalAssetsUSD,
                toCollectARS, toCollectUSD,
                debtsARS, debtsUSD,
                payARS, payUSD,
                fixedARS, fixedUSD,
                netoARS: patrimonioNetoARS,
                netoUSD: patrimonioNetoUSD
            }
        };
    }, [data]);
    ...
    const handleAssetSubmit = async (formData) => {
        ...
        try {
            ...
            setActionError(error.message || 'No se pudo guardar el auto personal.');
        } finally {
            setSavingAction(false);
        }
    };
    ...
    const renderTabPanel = () => {
        ...
        if (activeTab === 'Mis autos') {
            const legacyPersonalCars = summary.personalTasks.personalCar || [];
            return (
                <>
                    <PanelHeader title={`Mis autos personales — ${summary.personalCars.length} registrados`} action={<ActionButton onClick={() => setAssetModalOpen(true)}>Nuevo auto personal</ActionButton>} />
                    
                    <ListPanel
                        items={summary.personalCars}
                        emptyTitle="Sin autos personales"
                        emptyText="Todavía no cargaste autos personales."
                        onEmptyAction={() => setAssetModalOpen(true)}
                        renderItem={(asset) => (
                            <SimpleRow
                                key={asset._id}
                                keyValue={asset._id}
                                title={`${asset.brand} ${asset.model} ${asset.year ? asset.year : ''}`}
                                meta={`Patente: ${asset.plate || 'Sin patente'} ${asset.notes ? `• ${asset.notes}` : ''}`}
                                amount={money(asset.estimatedValue, asset.currency)}
                            />
                        )}
                    />

                    {legacyPersonalCars.length > 0 && (
                        <div className="mt-8">
                            <SectionTitle>Autos personales legacy</SectionTitle>
                            <p className="mb-4 text-xs text-crm-fg-muted">Estos registros fueron cargados con el formato anterior. Volvé a cargarlos como auto personal para tener valor, patente y datos completos.</p>
                            <div className="space-y-2 opacity-75 grayscale transition-all hover:opacity-100 hover:grayscale-0">
                                {legacyPersonalCars.map(renderPersonalTaskRow)}
                            </div>
                        </div>
                    )}
                </>
            ...
            return (
                <>
                    ...
                    <section className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <BalanceCard label="Patrimonio Neto USD" value={money(summary.patrimonio.netoUSD, 'USD')} tone={summary.patrimonio.netoUSD >= 0 ? 'green' : 'red'} />
                        <BalanceCard label="Patrimonio Neto ARS" value={money(summary.patrimonio.netoARS, 'ARS')} tone={summary.patrimonio.netoARS >= 0 ? 'green' : 'red'} />
                    </section>

                    <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
                        <div>
                            <SectionTitle>Composición USD</SectionTitle>
                            <div className="space-y-2">
                                <SimpleRow keyValue="assets-usd" title="Autos Personales" amount={money(summary.patrimonio.totalAssetsUSD, 'USD')} />
                                <SimpleRow keyValue="collect-usd" title="Cuotas a Cobrar" amount={money(summary.patrimonio.toCollectUSD, 'USD')} />
                                <SimpleRow keyValue="debts-usd" title="Deudas" amount={`-${money(summary.patrimonio.debtsUSD, 'USD')}`} />
                                <SimpleRow keyValue="pay-usd" title="Cuotas a Pagar" amount={`-${money(summary.patrimonio.payUSD, 'USD')}`} />
                            </div>
                        </div>
                        <div>
                            <SectionTitle>Composición ARS</SectionTitle>
                            <div className="space-y-2">
                                <SimpleRow keyValue="assets-ars" title="Autos Personales" amount={money(summary.patrimonio.totalAssetsARS, 'ARS')} />
                                <SimpleRow keyValue="collect-ars" title="Cuotas a Cobrar" amount={money(summary.patrimonio.toCollectARS, 'ARS')} />
                                <SimpleRow keyValue="debts-ars" title="Deudas" amount={`-${money(summary.patrimonio.debtsARS, 'ARS')}`} />
                                <SimpleRow keyValue="pay-ars" title="Cuotas a Pagar" amount={`-${money(summary.patrimonio.payARS, 'ARS')}`} />
                            </div>
                        </div>
                    </div>

                    <div className="mt-8">
                        <SectionTitle>Compromisos Mensuales (Gastos Fijos)</SectionTitle>
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <SimpleRow keyValue="fixed-usd" title="Carga mensual USD" amount={money(summary.patrimonio.fixedUSD, 'USD')} />
                            <SimpleRow keyValue="fixed-ars" title="Carga mensual ARS" amount={money(summary.patrimonio.fixedARS, 'ARS')} />
                        </div>
                        <p className="mt-2 text-xs text-crm-fg-muted">Estos ítems representan carga mensual y no se restan del patrimonio neto absoluto.</p>
                    </div>
                </>
            );
        }
        ...
        if (activeTab === 'Contactos') {
            ...
            return (
                <>
                    <PanelHeader title={`Mis contactos clave — ${contacts.length} registrados`} subtitle="Tu agenda personal — separada de los clientes de la agencia." action={<ActionButton onClick={() => openTaskModal('contact')}>Nuevo teléfono</ActionButton>} />
                    <ListPanel
                        items={contacts}
