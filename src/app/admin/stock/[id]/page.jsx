...
export default function VehicleDetailPage() {
    ...
    const handleAddNote = async (note) => {
        ...
        try {
            ...
            const response = await fetch(baseUrl + '/api/admin/cars/' + params.id + '/notes', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + token
            ...
            if (!response.ok) {
                throw new Error(data.message || 'Error al guardar observacion');
            }

            toast.success('Observacion guardada');
            await fetchCar();
        } catch (err) {
            toast.error(err.message || 'Error al guardar observacion');
        }
    };
        ...
        return (
            <div className="flex items-center justify-center h-[50vh]">
                <div className="flex flex-col items-center gap-3 text-center">
                    <span className="text-crm-red font-bold">Error de conexión</span>
    ...
    return (
        <>
                <div className="flex flex-col gap-6 crm-no-print">
                    <VehicleDetailHeader vehicle={vehicle} activeReservation={activeReservation} />
                    
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        {/* Columna Principal (2/3) */}
                        <div className="lg:col-span-2 flex flex-col gap-6">
                            <VehicleFinancialSummary vehicle={vehicle} onEdit={() => setIsEditOpen(true)} />
                            <VehicleImagesPanel vehicle={vehicle} onSaveComplete={fetchCar} />
                            <VehicleInfoPanel vehicle={vehicle} onEdit={() => setIsEditOpen(true)} />
                        ...
                        <div className="flex flex-col gap-6">
                            ...
                            <VehicleActionsPanel 
                                ...
                                onStartSale={() => router.push('/admin/ventas')}
                                onPrintFlyer={handlePrint}
                ...
                <InputModal
                    ...
                    message="La nota se guardara como observacion interna del vehiculo."
                    label="Observacion"
                    placeholder="Escribi la observacion..."
                    confirmText="Guardar"
