...
import QuoteAuditTimeline from '../../../../components/crm/quotes/QuoteAuditTimeline';
...
export default function QuoteDetailPage({ params }) {
    ...
    const [quote, setQuote] = useState({
        ...
        paymentTerms: '',
        notes: '',
        tradeIn: { brand: '', model: '', year: '', plate: '', mileage: '', condition: '', value: 0, currency: 'USD' },
        status: 'pendiente',
        ...
        const init = async () => {
            try {
                ...
                if (!isNew) {
                    ...
                    if (quoteRes.ok) {
                        const data = await parseResponseSafe(quoteRes);
                        setQuote({
                            ...data,
    ...
    const handleSave = async () => {
        if (!quote.clientId || !quote.price || !quote.issueDate) {
            toast.error('Cliente, Precio y Fecha de Emisión son obligatorios.');
            return;
        }
            ...
            <div className="bg-crm-surface border border-crm-border rounded-xl p-8 shadow-sm print:shadow-none print:border-none print:bg-white print:text-black">
                ...
                <div className="flex justify-between items-start border-b border-crm-border pb-6 mb-6 print:border-gray-200">
                    <div>
                        <h1 className="text-2xl font-bold text-white print:text-black flex items-center gap-2">
                            ...
                            {isNew ? 'Nueva Cotización' : quote.quoteNumber ? `Cotización #${quote.quoteNumber}` : 'Cargando...'}
                        </h1>
                    ...
                    <div className="text-right">
                        ...
                        <div className="flex flex-col gap-2 mt-4 print:mt-2">
                            <div className="flex flex-col text-sm">
                                <label className="text-xs text-crm-fg-muted print:text-gray-500 mb-1">Fecha Emisión *</label>
                                <input type="date" value={quote.issueDate} onChange={e => handleChange('issueDate', e.target.value)} className="bg-crm-bg border border-crm-border text-white text-sm rounded-lg px-2 py-1 print:hidden" />
                                <span className="hidden print:block text-gray-600">Emisión: {new Date(quote.issueDate).toLocaleDateString()}</span>
                            </div>
                            <div className="flex flex-col text-sm">
                                <label className="text-xs text-crm-fg-muted print:text-gray-500 mb-1">Vencimiento</label>
                                <input type="date" value={quote.validUntil} onChange={e => handleChange('validUntil', e.target.value)} className="bg-crm-bg border border-crm-border text-white text-sm rounded-lg px-2 py-1 print:hidden" />
                                <span className="hidden print:block text-gray-600">Válida hasta: {new Date(quote.validUntil).toLocaleDateString()}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        <div className="mt-4">
                            <label className="text-xs text-crm-fg-muted print:text-gray-500 block mb-1">Vendedor</label>
                            <select 
                            ...
                            <div className="hidden print:block font-bold mt-1">
                                Vendedor: {sellers.find(s => s._id === quote.assignedTo)?.firstName} {sellers.find(s => s._id === quote.assignedTo)?.lastName || 'No asignado'}
                            </div>
                        </div>
                    </div>
                ...
                <div className="mb-8">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
                        ...
                        <div>
                            <label className="text-xs text-crm-fg-muted print:text-gray-500 block mb-1">Estado</label>
                            <select value={quote.tradeIn?.condition || ''} onChange={e => handleChange('tradeIn.condition', e.target.value)} className="w-full bg-crm-bg border border-crm-border rounded p-2 text-white print:hidden">
                                <option value="">Select...</option>
                                <option value="excelente">Excelente</option>
                                <option value="bueno">Bueno</option>
                                <option value="regular">Regular</option>
                            </select>
                            <div className="hidden print:block text-sm">{quote.tradeIn?.condition || '--'}</div>
                        </div>
                        <div>
                            <label className="text-xs text-crm-fg-muted print:text-gray-500 block mb-1">Patente</label>
                            <input type="text" value={quote.tradeIn?.plate || ''} onChange={e => handleChange('tradeIn.plate', e.target.value)} className="w-full bg-crm-bg border border-crm-border rounded p-2 text-white print:hidden uppercase" />
                            <div className="hidden print:block text-sm">{quote.tradeIn?.plate || '--'}</div>
                        </div>
                        <div className="md:col-span-2">
                ...
                <div className="hidden print:flex justify-between items-end mt-16 pt-8 border-t border-gray-300">
                    <div className="text-xs text-gray-500">
                        ...
                        <p>Cotización válida hasta: {new Date(quote.validUntil).toLocaleDateString()}</p>
                    </div>
                    <div className="w-48 text-center border-t border-gray-400 pt-2 text-sm">
                        Firma Vendedor
                    </div>
                    <QuoteAuditTimeline quote={quote} />
                </div>
            </div>

            <ConfirmModal
                isOpen={confirmDuplicateModal}
                onClose={() => setConfirmDuplicateModal(false)}
                onConfirm={confirmDuplicate}
                title="Nueva revisión"
                message="¿Desea crear una nueva revisión a partir de esta cotización?"
                confirmText="Crear revisión"
                isDestructive={false}
            />
        </div>
    );
