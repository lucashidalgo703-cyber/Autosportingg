...
export default function ReminderModal({ isOpen, onClose, installment, onSave }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-crm-bg border border-crm-border rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]">
                
                {/* Header */}
                <div className="flex justify-between items-center p-5 border-b border-crm-border shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-crm-fg-muted">Agendar tarea de seguimiento de cobranza</p>
                        </div>
                    </div>
                    <button onClick={onClose} className="p-2 text-crm-fg-muted hover:text-white hover:bg-crm-surface-raised rounded-lg transition-colors">
                        <X size={20} />
                    </button>
                </div>
                    ...
                    <form id="reminder-form" onSubmit={handleSubmit} className="flex flex-col gap-5">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className="w-full bg-black/40 border border-crm-border rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-orange-500 transition-colors"
                            />
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full bg-black/40 border border-crm-border rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-orange-500 transition-colors [color-scheme:dark]"
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-neutral-400 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                                ...
                                {['baja', 'media', 'alta'].map(p => (
                                    <button
                                        ...
                                        className={`py-2 rounded-xl text-sm font-bold capitalize transition-colors border ${
                                            ...
                                            : 'bg-black/40 text-neutral-400 border-crm-border hover:bg-crm-surface-raised'
                                        }`}
                                    >
                                        {p}
                                    </button>
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                rows={5}
                                className="w-full bg-black/40 border border-crm-border rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-orange-500 transition-colors resize-none"
                            />
                        </div>
                ...
                {/* Footer */}
                <div className="p-5 border-t border-crm-border flex justify-end gap-3 shrink-0 bg-crm-bg rounded-b-2xl">
                    <button
                        type="button"
                        onClick={onClose}
                        disabled={isSubmitting}
                        className="px-5 py-2.5 rounded-xl text-sm font-bold text-white hover:bg-crm-surface-raised transition-colors disabled:opacity-50"
                    >
                        Cancelar
                    </button>
                    <button
