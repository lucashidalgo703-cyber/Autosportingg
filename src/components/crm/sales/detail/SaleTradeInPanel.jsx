...
export default function SaleTradeInPanel({ sale, onUpdate }) {
    ...
    const [confirmStockModal, setConfirmStockModal] = useState({ isOpen: false, index: null });
    ...
    const handleEnterStock = (index) => {
        ...
        setConfirmStockModal({ isOpen: true, index });
    };

    const confirmEnterStock = async () => {
        const index = confirmStockModal.index;
        ...
        try {
            ...
            setConfirmStockModal({ isOpen: false, index: null });
        }
    };

    return (
        <div className="bg-neutral-900 border border-neutral-800 rounded-2xl overflow-hidden flex flex-col h-full">
            <div className="p-4 border-b border-neutral-800 flex items-center justify-between bg-crm-surface">
            ...
            <div className="p-5 flex-1 overflow-y-auto custom-scrollbar">
                {isEditing ? (
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="bg-crm-surface border border-neutral-800 rounded-xl p-3 flex justify-between items-center mt-4">
            ...
            <ConfirmModal
                isOpen={confirmStockModal.isOpen}
                onClose={() => setConfirmStockModal({ isOpen: false, index: null })}
                onConfirm={confirmEnterStock}
                title="Ingresar a Stock"
                message="¿Estás seguro de ingresar este vehículo al stock? Se creará una nueva unidad oculta."
                confirmText="Ingresar Vehículo"
                isDestructive={false}
            />
        </div>
    );
