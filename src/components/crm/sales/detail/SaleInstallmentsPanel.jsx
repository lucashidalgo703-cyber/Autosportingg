...
export default function SaleInstallmentsPanel({ sale, saleFinanceData }) {
    ...
    const [confirmDeleteSingleModal, setConfirmDeleteSingleModal] = useState({ isOpen: false, inst: null });
    const [confirmDeletePlanModal, setConfirmDeletePlanModal] = useState(false);
    const [confirmPaymentStatusModal, setConfirmPaymentStatusModal] = useState({ isOpen: false, data: null });
    ...
    const handleDeleteSingle = (inst) => {
        if (inst.status !== 'anulada') return;
        setConfirmDeleteSingleModal({ isOpen: true, inst });
    };

    const confirmDeleteSingle = async () => {
        const { inst } = confirmDeleteSingleModal;
        if (!inst) return;
        try {
            await deleteInstallment(inst._id);
            await loadData();
        } catch (err) {
            toast.error(err.message || 'Error al eliminar la cuota');
        } finally {
            setConfirmDeleteSingleModal({ isOpen: false, inst: null });
        }
    };
    ...
    const handleDeletePlan = () => {
        setConfirmDeletePlanModal(true);
    };
    ...
    const handleSaveTransaction = async (data) => {
        try {
            ...
            setConfirmPaymentStatusModal({ isOpen: true, data });
        } catch (err) {
            console.error(err.message);
        }
    };
    ...
    const confirmPaymentStatus = async () => {
        const { data } = confirmPaymentStatusModal;
        if (!data) return;
        try {
            ...
            setConfirmPaymentStatusModal({ isOpen: false, data: null });
        }
    };
    ...
    return (
        <div className="bg-crm-surface border border-neutral-800 rounded-2xl overflow-hidden mt-6">
            ...
            <ConfirmModal
                isOpen={confirmDeleteSingleModal.isOpen}
                onClose={() => setConfirmDeleteSingleModal({ isOpen: false, inst: null })}
                onConfirm={confirmDeleteSingle}
                title="Eliminar Cuota"
                message="¿Estás seguro de que quieres eliminar esta cuota anulada permanentemente?"
                confirmText="Eliminar"
                isDestructive={true}
            />

            <ConfirmModal
                isOpen={confirmDeletePlanModal}
                onClose={() => setConfirmDeletePlanModal(false)}
                onConfirm={confirmDeletePlan}
                title="Eliminar Plan Completo"
                message="¿Estás seguro de que quieres eliminar TODO el plan de cuotas? Esta acción es irreversible."
                confirmText="Eliminar Plan"
                isDestructive={true}
            />

            <ConfirmModal
                isOpen={confirmPaymentStatusModal.isOpen}
                onClose={() => setConfirmPaymentStatusModal({ isOpen: false, data: null })}
                onConfirm={confirmPaymentStatus}
                title="Marcar Cuota como Pagada"
                message="Movimiento registrado. ¿Querés marcar esta cuota como pagada_manual?"
