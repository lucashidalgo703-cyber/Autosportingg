    ...
    const CarList = ({ items, valueFormatter, valueColor }) => {
        ...
        return (
            <div className="space-y-3">
                {items.map((car, index) => (
                    <div key={car.id} className="flex items-center justify-between p-3 rounded-lg border border-crm-border bg-crm-bg hover:bg-crm-surface transition-colors group">
                        <div className="flex items-center gap-3 overflow-hidden">
                            <span className="text-xs font-bold text-crm-fg-muted w-4">{index + 1}.</span>
                            {car.coverImage ? (
                                <div className="w-8 h-8 rounded bg-crm-bg overflow-hidden shrink-0">
