...
function DeadlinesPanel() {
    return (
        <Panel title="Proximas entregas y vencimientos" subtitle="Entregas, expedientes y cuotas - proximos 7 dias" href="/admin/agenda" icon={Timer} className="min-h-[399px]">
...
function InstallmentsMonthPanel({ metrics }) {
    ...
    return (
        <Panel title="Cuotas a pagar - este mes" subtitle={`${cuotas.cantidadMes} cuotas pendientes del mes`} href="/admin/finanzas?tab=cuotas" icon={CreditCard} className="min-h-[460px]">
...
export default function GeneralDashboardSote({ metrics, canSeeFinancials = false, user, hideAmounts = false, selectedDate }) {
    ...
    const primaryCards = [
        {
            ...
            detail: `+ USD ${formatCurrency(estimatedMarginUsd)}`,
            icon: DollarSign,
            tone: 'green',
            href: '/admin/ventas'
        },
        {
            ...
            detail: `+ USD ${formatCurrency(activeCapitalUsd)} · ${activeStock} veh.`,
            icon: Warehouse,
            tone: 'blue',
            href: '/admin/stock'
        },
