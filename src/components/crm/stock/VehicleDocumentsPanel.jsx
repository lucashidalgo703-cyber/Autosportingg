...
export default function VehicleDocumentsPanel({ vehicle, onSaveComplete }) {
    ...
    const getStatusIcon = (status) => {
        switch(status) {
            ...
            case 'pendiente': return <FileX size={18} className="text-crm-red" />;
    ...
    const handleSave = async () => {
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(errorData.message || errorData.error || 'Error al guardar documentacion');
            }
        ...
        <CrmCard>
            ...
            <div className="flex flex-col gap-2">
                {Object.keys(labels).map((key) => (
                    <div key={key} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-crm-bg border border-crm-border rounded-lg gap-3">
            ...
            {isEditing && (
                <div className="flex justify-end gap-2 mt-4 pt-4 border-t border-crm-border">
                    <button 
                        ...
                        disabled={loading}
                        className="px-3 py-1.5 text-xs text-white bg-crm-surface-raised hover:bg-crm-surface-raised rounded transition-colors"
                    >
                        Cancelar
                    </button>
                    <button 
