...
export default function VehicleHistoryTimeline({ vehicle }) {
    ...
    return (
        <CrmCard>
            ...
            {timeline.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-8 text-center bg-crm-bg rounded-xl border border-crm-border border-dashed">
                    <Clock size={24} className="text-crm-fg-muted mb-2 opacity-50" />
                    <p className="text-sm text-crm-fg-muted">Todavía no hay movimientos registrados para este vehículo.</p>
                </div>
            ) : (
                <div className="flex flex-col relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-crm-surface-raised gap-6">
                    {timeline.map((item, index) => {
                        ...
                        return (
                            <div key={item._id || index} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                                <div className={`flex items-center justify-center w-10 h-10 rounded-full border-4 border-crm-border ${color} text-white shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow z-10`}>
                                    <Icon size={16} />
                                </div>
                                <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-crm-bg p-4 rounded-xl border border-crm-border">
                                    <div className="flex justify-between items-center mb-2">
                                        <div className="flex items-center gap-2">
                                            <span className="px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded bg-crm-surface-raised text-white">
                                                {item.action}
                                            </span>
                                            <span className="text-[10px] text-crm-fg-muted font-medium">
                                                {item.user}
                                            </span>
                                        </div>
                                        <span className="text-xs text-crm-fg-muted flex items-center gap-1 shrink-0">
                                            <Clock size={12}/> 
                                    ...
                                    {(item.oldValue !== undefined && item.newValue !== undefined && item.action !== 'GASTO') && (
                                        <div className="mt-3 p-2 bg-crm-bg rounded border border-crm-border grid grid-cols-2 gap-2 text-xs">
                                            <div className="flex flex-col gap-0.5 overflow-hidden">
                                                <span className="text-[10px] text-crm-fg-muted uppercase tracking-wide">Anterior</span>
                                                <span className="text-[#EF4444] line-through truncate" title={String(item.oldValue)}>
                                                    {String(item.oldValue) || '-'}
                                                </span>
                                            </div>
                                            <div className="flex flex-col gap-0.5 overflow-hidden">
                                                <span className="text-[10px] text-crm-fg-muted uppercase tracking-wide">Nuevo</span>
