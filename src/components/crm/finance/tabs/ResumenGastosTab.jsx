...
import { FileSpreadsheet, Plus, Edit, Trash2 } from 'lucide-react';

export default function ResumenGastosTab({ allTransactions = [], onEdit, onDelete, onNew }) {
    const egresosTransactions = useMemo(() => {
    ...
    const metrics = useMemo(() => {
        let totalARS = 0;
        let totalUSD = 0;
        egresosTransactions.forEach(tx => {
    ...
    return (
        <div className="space-y-4">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="rounded-xl border border-crm-border bg-crm-bg p-4 flex flex-col justify-between">
                    <h3 className="text-[11px] font-black uppercase tracking-wider text-crm-fg-subtle">TOTAL GASTOS (ARS)</h3>
                    <p className="mt-2 text-xl font-black text-crm-red">{formatMoney(metrics.totalARS, 'ARS')}</p>
                </div>
                <div className="rounded-xl border border-crm-border bg-crm-bg p-4 flex flex-col justify-between">
                    <h3 className="text-[11px] font-black uppercase tracking-wider text-crm-fg-subtle">TOTAL GASTOS (USD)</h3>
                    <p className="mt-2 text-xl font-black text-crm-red">{formatMoney(metrics.totalUSD, 'USD')}</p>
                </div>
            </div>
            ...
            <section className="rounded-2xl border border-crm-border bg-crm-surface p-4">
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div className="flex flex-wrap items-center gap-1">
                        <button className="h-9 rounded-none border-b-2 border-crm-red px-2 text-sm font-black text-crm-red transition">RESUMEN DE GASTOS</button>
                    </div>
                    ...
                    <table className="w-full text-left border-collapse whitespace-nowrap">
                        <thead className="bg-crm-surface-raised border-b border-crm-border">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-[10px] font-black uppercase tracking-wider text-crm-fg-muted">Descripción / Origen</th>
                                ...
                                <tr>
                                    <td colSpan={6} className="p-8 text-center">
                                        <div className="flex flex-col items-center justify-center gap-3">
                                            ...
                                            <p className="text-sm text-crm-fg-muted max-w-sm">No se encontraron egresos registrados en el sistema.</p>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
