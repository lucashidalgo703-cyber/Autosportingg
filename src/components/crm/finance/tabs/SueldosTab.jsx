...
export default function SueldosTab({ allTransactions = [], onNew, onEdit, onDelete }) {
    // Filtrar solo los egresos que pertenezcan a la categoría sueldos (case insensitive)
    const sueldosTransactions = useMemo(() => {
        return allTransactions.filter(tx => 
            tx.status !== 'anulado' && 
            tx.type?.toLowerCase() === 'egreso' &&
            (tx.category?.toLowerCase().includes('sueldo') || tx.concept?.toLowerCase().includes('sueldo'))
    ...
    const metrics = useMemo(() => {
        let totalARS = 0;
        let totalUSD = 0;
        sueldosTransactions.forEach(tx => {
        ...
        <div className="space-y-4">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="rounded-xl border border-crm-border bg-crm-bg p-4">
                    <h3 className="text-[11px] font-black uppercase tracking-wider text-crm-fg-subtle">TOTAL SUELDOS PAGADOS (ARS)</h3>
                    <p className="mt-2 text-xl font-black text-crm-fg">{formatMoney(metrics.totalARS, 'ARS')}</p>
                </div>
                <div className="rounded-xl border border-crm-border bg-crm-bg p-4">
                    <h3 className="text-[11px] font-black uppercase tracking-wider text-crm-fg-subtle">TOTAL SUELDOS PAGADOS (USD)</h3>
            ...
            <section className="rounded-2xl border border-crm-border bg-crm-surface p-4">
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div className="flex flex-wrap items-center gap-1">
                        <button className="h-9 rounded-none border-b-2 border-crm-red px-2 text-sm font-black text-crm-red transition">REGISTRO DE SUELDOS</button>
                    </div>
                ...
                <div className="overflow-x-auto custom-scrollbar">
                    <table className="w-full text-left border-collapse whitespace-nowrap">
                        ...
                        <tbody className="divide-y divide-crm-border">
                            {sueldosTransactions.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="p-8 text-center">
                                        <div className="flex flex-col items-center justify-center gap-3">
                                            <div className="h-12 w-12 rounded-full bg-crm-bg flex items-center justify-center mb-2">
                                                <Users className="text-crm-fg-muted" size={24} />
                                            </div>
                                            <h4 className="font-bold text-crm-fg">Sin sueldos registrados</h4>
                                            <p className="text-sm text-crm-fg-muted max-w-sm">No se encontraron egresos con la categoría "Sueldos".</p>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                                sueldosTransactions.map(tx => (
